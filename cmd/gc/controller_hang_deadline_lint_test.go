...
var controllerTestExcludedHangDeadlineLines = map[int]string{
	422:  "input the test feeds a fake server to define the scenario, not a hang detector",
	878:  "negative-assertion window (asserts no watcher poke arrives)",
	928:  "negative-assertion window (asserts no watcher poke arrives, loop body)",
	1457: "bounded best-effort probe with no assertion on either branch",
}
